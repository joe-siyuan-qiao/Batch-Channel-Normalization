...
class PBCNorm(nn.Module):

    def __init__(self, num_channels, num_groups, momentum, eps=1e-5, with_pbn=True):
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes, num_groups=None, weight_std=False, beta=False):
        ...
        self.norm = lambda planes, momentum=0.05: PBCNorm(num_groups=32, num_channels=planes, momentum=momentum)
...
def resnet101(pretrained=False, num_groups=None, weight_std=False, **kwargs):
    ...
    if pretrained:
        ...
        if num_groups and weight_std:
            ...
            pretrained_dict = torch.load('data/rn101-bcn-ws.pth.tar')
            overlap_dict = {k[7:]: v for k, v in pretrained_dict.items() if k[7:] in model_dict}
            # assert len(overlap_dict) == 312
            print ('Loaded {} parameters'.format(len(overlap_dict)))
